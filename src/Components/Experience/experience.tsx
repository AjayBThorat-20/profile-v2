...
export default function Experience() {
  ...
  useEffect(() => {
    if (isHovered !== null) {
      const interval = setInterval(() => {
        setCurrentImageIndexes((prevIndexes) => {
          const newIndexes = [...prevIndexes];
          newIndexes[isHovered] = (newIndexes[isHovered] + 1) % experienceData[isHovered].details.length;
          return newIndexes;
        });
      }, 2000);
      return () => clearInterval(interval);
  ...
  const handleViewDetails = (experience: (typeof experienceData)[number]) => {
    localStorage.setItem("selectedExperience", JSON.stringify(experience));
    router.push(`/experience/details?id=${experience.id}&company=${encodeURIComponent(experience.name)}`);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {experienceData.map((experience, idx) => (
            ...
            >
              ...
              <div className="p-6">
                <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200">{experience.title}</h2>
                <p className="text-gray-600 dark:text-gray-400 mt-2">{experience.name}</p>
                <p className="text-gray-600 dark:text-gray-400 mt-2">{experience.duration}</p>
