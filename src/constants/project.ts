  ...
  export const projectsData: readonly ProjectItem[] = [
    {
      ...
      discription:
        "DevCompass is a production npm CLI tool I published and actively maintain for JavaScript dependency health analysis, combining real-time CVE vulnerability scanning through OSV and NVD database integration with AI-powered recommendations across four LLM providers (OpenAI, Anthropic, Google Gemini, and local Ollama). I engineered an interactive dependency graph dashboard using D3.js with five distinct layouts (tree, force-directed, radial, conflict-highlighting, and analytics) featuring real-time search and filtering, plus an intelligent auto-fix system with risk classification, dry-run previews, automatic backups, a migrate-syntax codemod engine for major-version upgrades, and a one-command undo for any fix session. Past security hardening passes fixed a shell-injection vulnerability and a path-traversal gap, upgraded API key encryption from unsalted SHA-256 to salted scrypt, moved every remaining exec call site from shell string interpolation to execFile with argv arrays so shell interpretation is impossible by construction, and closed a token-leak path where a GitHub API token could follow a redirect to a third-party host. CI now runs CodeQL and OpenSSF Scorecard scans, enforces a 60% line-coverage floor, and gates every release on a version-guard job that checks the tag against what's already published on npm. Now on v4.1.8 with 50 published releases and a self-analyzed health score of 9.3/10, with full documentation at devcompass.ajaythorat.com.",
      pictures: [
